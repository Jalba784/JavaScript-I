...
// let carModels = inventory.car_model.sort();
let carModels = [];
...
let carYears = [];
for (let i = 0; i < inventory.length; i++) {
    carYears.unshift(inventory[i].car_year);
}
console.log(carYears);

// ==== Challenge 5 ====
// The car lot manager needs to find out how many cars are older than the year 2000. Using the carYears array you just
// created, find out how many cars were made before the year 2000 by populating the array oldCars and logging it's length.
let oldCars = [];
...
console.log(`${oldCars.length} cars from the inventory were made before the year 2000.`);

// ==== Challenge 6 ====
// A buyer is interested in seeing only BMW and Audi cars within the inventory.  Return an array that only contains BMW and
// Audi cars.  Once you have populated the BMWAndAudi array, use JSON.stringify() to show the results of the array in the console.
let BMWAndAudi = [];
for (let i = 0; i < inventory.length; i++) {
